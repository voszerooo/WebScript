...
//                                 if (el) {
//                                     runAction(el, action);
//                                     VBN_GLOBAL_DEBUG.log(moduleMark, `Step${i + 1} ✅ ${action}`, { debug });
//                                 } else {
//                                     VBN_GLOBAL_DEBUG.warn(moduleMark, `Step${i + 1} ❓ None`, { debug });
//                                     if (strict) shouldBreak = true;
//                                 }
//                             } catch (e) {
//                                 VBN_GLOBAL_DEBUG.error(moduleMark, `Step${i + 1} ❌ ${e.message}`, { debug });
//                             }
...
//                 let list = Array.from(root.querySelectorAll(URL));
//                 if (!list.length) {
//                     log("❌ no items");
//                     return;
//                 }

//                 const maxCount = item > 0 ? item : limit;
//                 if (maxCount > 0) list = list.slice(0, maxCount);

//                 log("items:", list.length);

//                 // ⭐⭐⭐ 关键策略：前几个同步 click（最大化成功率）
//                 const SAFE_COUNT = 5; // 你可以调 3~10

//                 // 1️⃣ 同步点击（最关键）
//                 for (let i = 0; i < list.length && i < SAFE_COUNT; i++) {
//                     log("sync click:", i);
//                     list[i].click();
//                 }

//                 // 2️⃣ 剩余的延迟执行（可能被拦，但尽量）
//                 for (let i = SAFE_COUNT; i < list.length; i++) {
//                     setTimeout(() => {
//                         log("delay click:", i);
//                         list[i].click();
//                     }, delayStep * (i - SAFE_COUNT));
//                 }
//             };

//             const mountDOM = (cb) => {
//                 if (mount === "VBN" && typeof VBN_GLOBAL_DOM !== "undefined") {
//                     VBN_GLOBAL_DOM.load(cb);
//                 } else if (typeof VBN_GLOBAL_DOM !== "undefined") {
//                     VBN_GLOBAL_DOM.load(cb);
//                 } else {
//                     document.addEventListener("DOMContentLoaded", () => cb(document.body));
//                 }
//             };

//             mountDOM(dom => {

//                 let group = dom.querySelector(".vbnAutoExecuteBatch");

//                 if (!group) {
//                     group = document.createElement("div");
//                     group.className = "vbnAutoExecuteBatch";
...
// })();
    ...
    // $ ================================================== ↓ Assign
            ...
            VBN_GLOBAL_LOGIC.activate("Template", {
                ...
                if (!VBN_GLOBAL_LOGIC.activate("Template")) return;
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            ...
                            --vbnWidth: var(--vbnSpaceWidth02); /* XXXX */
                            --vbnHeight: 260px;                 /* XXXX */
                ...
                GM_addStyle( /* css */ `
                    ...
                    html:has(body [role="listitem"]:nth-child(2) a[aria-disabled="true"]) {
                        ...
                        /* ================================================== ↓ Head */
                            ...
                            .wZQcA .BO2cCe, .zLSRge { border-color: var(--vbnLine); }
            ...
            VBN_GLOBAL_LOGIC.activate("Chat", {
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        /* ========== ↓ 首页搜索框 */
                        ._9a2f8e4 { max-width: calc(var(--vbnWidth) + 5vw); }
                        
                        /* ========== ↓ 聊天容器 */
                        .ds-virtual-list { --message-list-max-width: calc(var(--vbnWidth) + 5vw); }
                        
                        @media (width <= 1920px) {
                            ._9a2f8e4 { max-width: calc(var(--vbnWidth) + 20vw); }
                            .ds-virtual-list { --message-list-max-width: calc(var(--vbnWidth) + 20vw); }
                        }
                        @media (width <= 1260px) {
                            ._9a2f8e4 { max-width: calc(var(--vbnWidth) + 40vw); }
                            .ds-virtual-list { --message-list-max-width: calc(var(--vbnWidth) + 40vw); }
                        }
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            ...
                            --vbnVideo: 6;                    /* 显示数量 Video */
                            --vbnShots: 8;                    /* 显示数量 Shots */
                            --vbnBlur: var(--vbnPanelFilter); /* 播放器 控件背景模糊值*/
                        ...
                        @media (width >= 1500px) {
                            ...
                            ytd-rich-grid-renderer[is-default-grid] ytd-rich-item-renderer[rendered-from-rich-grid] {
                                --ytd-rich-grid-items-per-row: var(--vbnVideo) !important;
                            }
                            
                            /* ========== ↓ Shots */
                            ytd-rich-shelf-renderer[is-shorts] ytd-rich-item-renderer[items-per-row][is-slim-media] {
                                --ytd-rich-grid-items-per-row: var(--vbnShots) !important;
                            }
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            ...
                            --vbnPlayList: calc(40vh + 2vh + 1vh);    /* 播放列表高度 */
                ...
                VBN_GLOBAL_LOGIC.activate("Other", {
                    ...
                    GM_addStyle( /* css */ `
                        ...
                        html {
                            ...
                            &.vbn body {
                                --vbnWidth: var(--vbnSpaceWidth02); // 页面宽度
                            }
                            
                            .layout.layout-reskin-wider {
                                --content-width: minmax(auto, var(--vbnWidth));
                            }
                ...
                VBN_GLOBAL_LOGIC.activate("Other", {
                    ...
                    (function VBN_RadBook_ReLimits() {
                        ...
                        ['contextmenu', 'selectstart', 'copy', 'cut', 'paste'].forEach(t =>
                            window.addEventListener(t, e => e.stopPropagation(), true)
                        );
                        reset();
                        
                        let isKeyPressed = false;
                        document.addEventListener('keydown', e => e.key === 'Alt' && (isKeyPressed = true), true);
                        document.addEventListener('keyup', e => e.key === 'Alt' && (isKeyPressed = false), true);
                        
                        const stopKeyPressed = e => isKeyPressed && e.stopImmediatePropagation();
                        document.addEventListener('pointerdown', stopKeyPressed, true);
                        document.addEventListener('touchstart', stopKeyPressed, true);
                        document.addEventListener('mousedown', e => {
                            if (!isKeyPressed) return;
                            e.stopImmediatePropagation();
                            const target = e.target;
                            if (target?.tagName === 'IMG') target.setAttribute('draggable', 'true');
                        }, true);
