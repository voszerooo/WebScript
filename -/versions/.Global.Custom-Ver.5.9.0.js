 ...
 * # 
 * # 完善封装 VBN_GLOBAL_DEBUG 与 VBN_GLOBAL_DISPATCH 中 逻辑划分清晰 UI面板状态灯是否需要变更
 * # 在 UI 面板中 VBN_VALUE_RUNTIME 同步状态灯
 * # 让 VBN_GLOBAL_DISPATCH 扩展为 可控制站点下各个功能执行顺序 或者按新模块封装来实现
 * # 
 * # 
 * + ========== ↓ NEW
 * + 
        ...
        /* ================================================== ↓ VBN */
            ...
            /* ============================== ↓ Panel */
                ...
                .vbnRelative {
                    ...
                    /* ========== ↓ A */
                        ...
                        .vbnPanelGroup:not(.Sub) > .vbnGroupHeader {
                            color: hsla(var(--vbnBase06HSL), .6);
                        }
                        
                    /* ========== ↓ B */
                    
                        .vbnPanelGroup.Sub > .vbnGroupHeader, .vbnPanelGroup > .vbnGroupBody {
                            color: hsla(var(--vbnBase05HSL), .9);
                        }
            ...
            /* ============================== ↓ Panel */
                ...
                /* ========== ↓ Other */
                    ...
                    :where(.vbnOptionBase) { vbn { top: var(); }
    ...
    // $ ================================================== ↓ Global
        ...
        // #region Value
            ...
            const VBN_VALUE_STORE = (() => {
                ...
                function store(prefix) {
                    ...
                    return {
                        ...
                        get(key, fallback) {
                            ...
                            const val = GM_getValue(getKey(key), fallback);
                            if (val !== undefined) {
                                cache.set(key, val);
                            }
                            return val;
                        ...
                        set(key, val) {
                            cache.set(key, val);
                            GM_setValue(getKey(key), val);
                        },
                        delete(key) {
                            cache.delete(key);
                            GM_deleteValue(getKey(key));
                        },
                        clear() { cache.clear(); },
                ...
                const prefs = store("prefs");   // 开关 / 配置
                const hover = store("hover");   // hover
                const fold  = store("fold");    // UI 折叠状态
            ...
            const VBN_VALUE_CACHE = (() => {
                ...
                function clear(type) {
                    ...
                    if (type === "regex") regex.clear();
                    if (type === "match") match.clear();
                    if (type === "group") group.clear();
                    if (type === "chain") chain.clear();
                }
                
                return { 
                    regex, match, group, chain,
                    clear,
                };
            ...
            const VBN_VALUE_RUNTIME = (() => {
                
                function get(key, fallback = true) {
                    return VBN_VALUE_STORE.prefs.get(key, fallback);
                }
                function set(key, value) {
                    VBN_VALUE_STORE.prefs.set(key, !!value);
                }
                
                function group(groupKey) { return get(groupKey, true); }
                function chain(keys) { return keys.every(k => get(k, true)); }
                
                function toggle(key) {
                    const val = get(key, true);
                    const next = !val;
                    set(key, next);
                    return next;
                }
                
                return { get, set, group, chain, toggle, };
        ...
        // #region Basics
            ...
            const VBN_GLOBAL_DOM = (() => {
                ...
                const VBN = createManager('vbnDOM');
                
                return {
                    
                    bindVBN(callback) { return VBN.bind(callback); },
...
const VBN_GLOBAL_BUTTON = (() => {
    
    function normalize(configs) {
        return Array.isArray(configs) ? configs : [configs];
    }

    function createButton(cfg, baseClass) {
        ...
        const defaultState = cfg.default ?? true;

        const state = VBN_VALUE_RUNTIME.get(cfg.key, defaultState);

        applyState(btn, cfg, state);

        btn.addEventListener("click", () => {

            const next = VBN_VALUE_RUNTIME.toggle(cfg.key);

            applyState(btn, cfg, next);
    ...
    function applyState(btn, cfg, state) {

        const names = normalize(cfg.name || ["ON", "OFF"]);
        const colors = normalize(cfg.color || ["var(--vbnAccentHEX)", "var(--vbnBase04HEX)"]);

        btn.textContent = state ? (names[0] || "ON") : (names[1] || "OFF");
        btn.style.background = state ? colors[0] : colors[1];

        btn.classList.toggle("vbnOpen", state);
        btn.classList.toggle("vbnClose", !state);
    }

    function render(dom, configs, baseClass, groupSelector) {

        let group = dom.querySelector(groupSelector);

        if (!group) {
            group = document.createElement("div");
            group.className = groupSelector.replace(".", "");
        ...
        configs.forEach(cfg => {
            ...
            // ✅ show 控制（你新增的）
            if (typeof cfg.show === "function" && !cfg.show()) return;

            if (group.querySelector(`.${cfg.key}`)) return;

            const btn = createButton(cfg, baseClass);
    ...
    function bindVBN(configs) {

        const list = normalize(configs);

        if (!VBN_GLOBAL_DOM || !VBN_GLOBAL_DOM.bindVBN) return;

        VBN_GLOBAL_DOM.bindVBN(dom => {
...
// VBN_GLOBAL_BUTTON.bindVBN({
//     key: "Button",
//     name: ["Pic.ON", "Pic.OFF"],
//     color: ["var(--vbnAccentHEX)", "var(--vbnBase04HEX)"],
//     class: ["vbnButtonBase"],

//     default: true,

//     shouldShow() {
//         return location.href.includes("viewthread");
//     },
...
// });
            ...
            const VBN_MATCH_RULE = (() => {
                ...
                function toRegex(pattern) {
                    ...
                    if (VBN_VALUE_CACHE.regex.has(pattern)) {
                        return VBN_VALUE_CACHE.regex.get(pattern);
                    }
                    ...
                    VBN_VALUE_CACHE.regex.set(pattern, regex);
                    return regex;
                ...
                function compile(matchRule = {}) {
                    
                    const include = matchRule.include || [];
                    const exclude = matchRule.exclude || [];
                    if (VBN_VALUE_CACHE.match.has(matchRule)) {
                        return VBN_VALUE_CACHE.match.get(matchRule);
                    }
                    const includeRegs = include.map(toRegex);
                    const excludeRegs = exclude.map(toRegex);
                    ...
                    const compiled = {
                        ...
                        test(url) {
                            ...
                            return (
                                includeRegs.some(r => r.test(url)) &&
                                !excludeRegs.some(r => r.test(url))
                            );
                        }
                        
                    };
                    
                    VBN_VALUE_CACHE.match.set(matchRule, compiled);
                    return compiled;
                }
                
                function match({ include = [], exclude = [], url = vbnMatchURL } = {}) {
                    const compiled = compile({ include, exclude });
                    const cacheKey = `${url}::${include.join("|")}::${exclude.join("|")}`;
                    
                    if (VBN_VALUE_CACHE.match.has(cacheKey)) {
                        return VBN_VALUE_CACHE.match.get(cacheKey);
                    }
                    const result = compiled.test(url);
                    VBN_VALUE_CACHE.match.set(cacheKey, result);
                    return result;
                }
                
                return { toRegex, check, match, compile, };
            ...
            const VBN_OBSERVER_CENTER = (() => {
                ...
                const observerTargetStats = new Map();
                ...
                function observe(target, options = { childList: true, subtree: true }, callback, config = {}) {
                    ...
                    const observerBase = new MutationObserver((mutations, observerInstance) => {
                        ...
                        if (config.debounce > 0) {
                            clearTimeout(timer);
                            timer = setTimeout(run, config.debounce);
                        } else {
                            run();
                        }
                    });
                    ...
                    addToTargetMap(target, observerBase);
                    return observerBase;
                }
                    ...
                    const observerBase = new MutationObserver((mutations, observerInstance) => {
                        const run = () => {
                            ...
                            if (
                                ...
                                disconnectKey(key);
                                removeFromTargetMap(target, observerBase);
                            }
                        };
                        if (config.debounce > 0) {
                            clearTimeout(timer);
                ...
                function disconnect(target) {
                    ...
                    if (observerSet) {
                        ...
                        observerTargetStats.delete(target);
                    }
                }
                ...
                function getActiveCount() {
                    let totalTargeted = 0;
                    observerTargetStats.forEach(set => totalTargeted += set.size);
                    return {
                ...
                function addToTargetMap(target, observerBase) {
                    
                    if (!observerTarget.has(target)) {
                        observerTarget.set(target, new Set());
                    }
                    if (!observerTargetStats.has(target)) {
                        observerTargetStats.set(target, new Set());
                    }
                    
                    observerTarget.get(target).add(observerBase);
                    observerTargetStats.get(target).add(observerBase);
                }
                
                function removeFromTargetMap(target, observerBase) {
                    const set1 = observerTarget.get(target);
                    if (set1) {
                        set1.delete(observerBase);
                        if (set1.size === 0) observerTarget.delete(target);
                    }
                    const set2 = observerTargetStats.get(target);
                    if (set2) {
                        set2.delete(observerBase);
                        if (set2.size === 0) observerTargetStats.delete(target);
                    }
                }
            ...
            const VBN_GLOBAL_INFORM = (() => {
                
                function createInform(bindFn, config = {}) {
                    ...
                    function hover(element, message, times, callback, delay = 926) {
                        ...
                        const shownCount = VBN_VALUE_STORE.hover.get(key) || 0;
                ...
                return {
                    VBN: createInform(VBN_GLOBAL_DOM.bindVBN, { informTime: 3000 }),
                };
            ...
            const VBN_GLOBAL_DEBUG = (() => {
                ...
                function setModuleState(key, { level, msg, time }) {
                    if (!moduleStateMap[key]) {
                        moduleStateMap[key] = {};
                    }
                    const mod = moduleStateMap[key];
                    mod.timeCost = time ?? mod.timeCost ?? 0;
                ...
                function infoTotal() {
                    for (const [key, mod] of Object.entries(moduleStateMap)) {
                        const icon =
                            mod.error ? "👾" :
                            mod.debugLevel === "warn" ? "🛠️" :
                            mod.debugLevel === "log" ? "🚀" : "🏁";
                        const msg =
                            mod.errorMSG ||
                            (mod.error ? "ERROR" : "DONE");
                            
                        const time = mod.timeCost || 0;
                        console.log(
                            logFormat({ icon, key, time, msg })
                        );
                    }
                ...
                return { log, warn, error, getLogs, infoTotal };
...
const VBN_GLOBAL_VIEW = (() => {

    function summary() {
        // 从 DEBUG 读取 moduleStateMap
    }
    ...
    return {
        ...
        // #region Core
            ...
            const VBN_MODULE_STORE = (() => {
                ...
                function moduleBase(key, raw, parentKey = null, groupName = null) {
                    return {
                        key,
                ...
                for (const [groupName, groupData] of Object.entries(VBN_GLOBAL_CONFIG)) {
                    ...
                    if (["debug"].includes(groupName)) continue;
                    const groupMod = moduleBase(groupName, groupData, null, groupName);
                    flat[groupName] = groupMod;
                    
                    const groupNode = {};
                    for (const [modKey, modRaw] of Object.entries(groupData)) {
                        
                        if (["fold", "debug", "block"].includes(modKey)) continue;
                        const mod = moduleBase(modKey, modRaw, groupName, groupName);
                        flat[modKey] = mod;
                        ...
                        for (const [subKey, subRaw] of Object.entries(modRaw)) {
                            ...
                            const subMod = moduleBase(subKey, subRaw, modKey, groupName);
                            flat[subKey] = subMod;
                    ...
                    tree[groupName] = {
                        ...groupNode,
                        fold: groupData.fold ?? false,
                    };
                }
                
                function getGroup(key) {
                    if (VBN_VALUE_CACHE.group.has(key)) {
                        return VBN_VALUE_CACHE.group.get(key);
                    }
                    const group = flat[key]?.group ?? null;
                    VBN_VALUE_CACHE.group.set(key, group);
                    return group;
                }
                
                function getChain(key, { includeGroup = false } = {}) {
                    
                    if (VBN_VALUE_CACHE.chain.has(key)) return VBN_VALUE_CACHE.chain.get(key);
                    const chain = [];
                    ...
                    if (includeGroup) {
                        ...
                        if (group && chain[0] !== group) {
                            chain.unshift(group);
                        }
                    }
                    VBN_VALUE_CACHE.chain.set(key, chain);
                    return chain;
            ...
            const VBN_ACTIVATION = (() => {
                ...
                function findMatch(mod) {
                    ...
                    if (VBN_VALUE_CACHE.match.has(mod.key)) {
                        return VBN_VALUE_CACHE.match.get(mod.key);
                    }
                    let result;
                    ...
                    VBN_VALUE_CACHE.match.set(mod.key, result);
                    return result;
                }
                
                function getCompiledMatch(mod, overrideMatch = null) {
                    if (overrideMatch) {
                        return VBN_MATCH_RULE.compile(overrideMatch);
                    }
                    if (VBN_VALUE_CACHE.regex.has(mod.key)) {
                        return VBN_VALUE_CACHE.regex.get(mod.key);
                    }
                    const matchRule = findMatch(mod);
                    const compiled = VBN_MATCH_RULE.compile(matchRule);
                    
                    VBN_VALUE_CACHE.regex.set(mod.key, compiled);
                    return compiled;
                }
                
                function getChain(mod) {
                    return VBN_MODULE_STORE.getChain(mod.key, { includeGroup: true });
                }
                
                function isBlocked(module, url = vbnMatchURL) {
                    const groupKey = VBN_MODULE_STORE.getGroup(module.key);
                    if (!groupKey) return false;
                    const groupMod = VBN_MODULE_STORE.get(groupKey);
                    const blockList = groupMod?.block;
                    
                    if (!blockList?.length) return false;
                    return blockList.some(pattern =>
                        VBN_MATCH_RULE.check(pattern, url)
                    );
                }
                
                function isActive(module, options = {}) {
                    const url = options.url ?? vbnMatchURL;
                    const overrideMatch = options.match || options.matchSubjoin || null;
                    const compiled = getCompiledMatch(module, overrideMatch);
                    if (!compiled.test(url)) return false;
                    
                    const keyChain = getChain(module);
                    return VBN_VALUE_RUNTIME.chain(keyChain);
                }
            ...
            const VBN_GLOBAL_DISPATCH = (() => {
                ...
                function getDebugState(mod) {
                    const groupKey = VBN_MODULE_STORE.getGroup(mod.key);
                    const groupMod = groupKey ? VBN_MODULE_STORE.get(groupKey) : null;
                    
                    return (
                        VBN_GLOBAL_CONFIG.debug ||
                        groupMod?.debug ||
                        mod.debug
                    );
                }
                ...
                function activate(key, rule, logic) {
                    ...
                    if (VBN_ACTIVATION.isBlocked(mod, options.url)) {
                        if (getDebugState(mod)) {
                            VBN_GLOBAL_DEBUG.warn(
                                key,
                                `block - ${VBN_MODULE_STORE.getGroup(key)}`
                            );
                        }
                        return false;
                    }
        ...
        // #region Utils
            ...
            const VBN_REMOVE_LIMITS = (() => {
                ...
                const drag = (isActive) => {
                    ...
                    VBN_OBSERVER_CENTER.observe(document.documentElement, {
                        childList: true,
                        subtree: true
                    }, run);
                };
                ...
                function apply({
                    ...
                    function loadSinglePage(pageNum) {
                        return new Promise(resolve => {
                            const time = VBN_RANDOM_JITTER.get({
                                jitter: finalJitter,
                                order: pageNum
                            });
                            setTimeout(() => {
...
const VBN_BATCH_LOAD = (() => {
    ...
    return { apply };
        ...
        // #region Derive
            ...
            const VBN = {
                ...
                VALUE: {
                    STORE: VBN_VALUE_STORE,
                    CACHE: VBN_VALUE_CACHE,
                    RUNTIME: VBN_VALUE_RUNTIME,             // &    VBN_VALUE_STORE
                },
                
                BASICS: {
                    DOM: VBN_GLOBAL_DOM,
                    // BUTTON: VBN_GLOBAL_BUTTON,              // &    VBN_GLOBAL_DOM         VBN_VALUE_RUNTIME
                    MATCH: VBN_MATCH_RULE,                  // &    VBN_VALUE_CACHE
                    OBSERVER: VBN_OBSERVER_CENTER,
                    RANDOM: VBN_RANDOM_JITTER,
                    INFORM: VBN_GLOBAL_INFORM,              // &    VBN_VALUE_STORE        VBN_GLOBAL_DOM
                    DEBUG: VBN_GLOBAL_DEBUG,                // &    VBN_GLOBAL_CONFIG       VBN_OBSERVER_CENTER
                    // VIEW: VBN_GLOBAL_VIEW,
                },
                
                CORE: {
                    STORE: VBN_MODULE_STORE,                // &    VBN_VALUE_CACHE        VBN_GLOBAL_CONFIG
                    ACTIVATION: VBN_ACTIVATION,             // &    VBN_VALUE_CACHE        VBN_MODULE_STORE        VBN_MATCH_RULE          VBN_VALUE_RUNTIME
                    DISPATCH: VBN_GLOBAL_DISPATCH,          // &    VBN_MODULE_STORE       VBN_ACTIVATION          VBN_GLOBAL_CONFIG       VBN_GLOBAL_DEBUG
                    // QUEUE: VBN_QUEUE_EXECUTE,               // &    VBN_GLOBAL_DEBUG
                },
                
                UTILS: {
                    CLASS: VBN_ADD_CLASS,                   // &    VBN_OBSERVER_CENTER
                    READ: VBN_QUICK_READ, 
                    REMOVE: VBN_REMOVE_LIMITS,              // &    VBN_OBSERVER_CENTER
                    LOAD: VBN_AUTO_LOAD,                    // &    VBN_RANDOM_JITTER
                    // BATCH: VBN_BATCH_LOAD,
                    EXECUTE: VBN_AUTO_EXECUTE,              // &    VBN_RANDOM_JITTER      VBN_GLOBAL_DEBUG
                },
                
            };
            
            VBN.CORE.DISPATCH.preheat();
        ...
        // #region Options
            ...
            (function VBN_Options() {
                ...
                function initFold(container, body, arrow, foldKey, initiallyFolded) {
                    ...
                    const toggleFold = () => {
                        ...
                        if (isCollapsed) {
                            ...
                            VBN_VALUE_STORE.fold.set(foldKey, false);
                            folded = false;
                        } else {
                            ...
                            VBN_VALUE_STORE.fold.set(foldKey, true);
                            folded = true;
                        }
                    };
                ...
                function buildSwitch(key, value, enabled, meta) {
                    ...
                    if (hoverText) VBN_GLOBAL_INFORM.VBN.hover(row, hoverText);
                ...
                function renderMod(modKey, mod, groupBody, groupEnabled) {
                    ...
                    if (["fold", "debug", "block"].includes(modKey) || typeof mod.state !== "boolean") return;
                    
                    const currentVal = VBN_VALUE_RUNTIME.get(modKey, mod.default ?? true);
                    ...
                    if (mod.subs || mod.fold) {
                        ...
                        if (!easterEggAttachedSet.has(toggleSubs)) {
                            VBN_GLOBAL_INFORM.VBN.hover(toggleSubs, "HEIHEI 彩蛋 | ClickClick", 2);
                        ...
                        if (hoverText) VBN_GLOBAL_INFORM.VBN.hover(label, hoverText);
                ...
                function buildPanel() {
                    ...
                    for (const [groupName, groupData] of Object.entries(allModules)) {
                        ...
                        const groupEnabled = VBN_VALUE_RUNTIME.get(groupKey, true);
                    ...
                    const applyBtn = createButton("Apply", "vbnButtonBase vbnButtonAdapt vbnOpen", () => {
                        for (const [key, value] of Object.entries(pendingChanges)) {
                            VBN_VALUE_RUNTIME.set(key, !!value);
                        }
                        VBN_GLOBAL_INFORM.VBN.top("已应用 即将刷新", "correct");
                    ...
                    const resetBtn = createButton("Reset", "vbnButtonBase vbnButtonAdapt vbnClose", () => {
                        ...
                        VBN_VALUE_CACHE.clear();
                        VBN_GLOBAL_INFORM.VBN.top("已重置 即将刷新", "correct");
                        setTimeout(() => location.reload(), 1260);
                    });
                    
                    panel.append(applyBtn, resetBtn);
                    
                    VBN_GLOBAL_DOM.bindVBN(dom => dom.appendChild(panel));
        ...
        // #region Geste
            ...
            (function VBN_Geste() {
                ...
                if (window.__VBN_GESTE_INITIALIZED__) return;
                window.__VBN_GESTE_INITIALIZED__ = true;
                ...
                const config = {
                    ...
                    effective: 5,               // 手势生效起始距离
                    maxDirection: 5,            // 滑动方向 X 次后无效
                    minDistance: 15,            // 最小触发距离
                    verticalThreshold: 50,      // 有效位移阈值
                    pageScrollRatio: .8,        // 上下滑滚动比率
                };
                ...
                function getMountContainer() {
                    if (typeof VBN_GLOBAL_DOM !== 'undefined' && VBN_GLOBAL_DOM.bindVBN) {
                        const vbn = document.getElementById('vbnDOM');
                        if (vbn) return vbn;
                    }
                    return document.getElementById('vbnDOM') || document.body;
                }
                ...
                function detectVShape(points) {
                    ...
                    let firstDir = null, secondDir = null, turnIndex = -1;
                    ...
                    const firstHalf = points.slice(0, turnIndex);
                    const secondHalf = points.slice(turnIndex);
                    const firstMove = Math.abs(firstHalf[firstHalf.length - 1].y - firstHalf[0].y);
                    const secondMove = Math.abs(secondHalf[secondHalf.length - 1].y - secondHalf[0].y);
                    if (firstMove < config.verticalThreshold || secondMove < config.verticalThreshold) return null;
                ...
                function isInteractiveElement(element) {
                    ...
                    if (role === 'textbox' || role === 'searchbox' || role === 'combobox' || role === 'button' || role === 'link') {
                        return true;
                    }
                ...
                function createCanvas() {
                    ...
                    `;
                    mountContainer.appendChild(newCanvas);
                    return newCanvas;
                }
                ...
                function onMouseMove(e) {
                    ...
                    drawTrail();
                    if (path.length >= 5) {
                        const currentDir = getDirection(dx, dy);
                        if (lastDirection && currentDir !== lastDirection) {
                            countDirection++;
                        }
                ...
                function onMouseUp(e) {
                    ...
                    const endX = path[path.length - 1]?.x ?? startX;
                    const endY = path[path.length - 1]?.y ?? startY;
                    const dx = endX - startX;
                    const dy = endY - startY;
                    const distance = Math.sqrt(dx * dx + dy * dy);
                    
                    if (gestureStarted && distance >= config.minDistance && countDirection <= config.maxDirection) {
                        const vShapeResult = detectVShape(path);
                        if (vShapeResult) {
                            executeAction(vShapeResult);
                        } else {
                ...
                function startGestureListener() {
                    ...
                    const styleId = 'vbn-gesture-style';
                ...
                setTimeout(startGestureListener, config.initDelay);
        ...
        // #region LoaderBar
            ...
            VBN_GLOBAL_DISPATCH.activate("LoaderBar", () => {
                ...
                (function VBN_LoaderBar() {
                    ...
                    function progressBarCreate() {
                        ...
                        VBN_GLOBAL_DOM.bindVBN(dom => dom.appendChild(loaderBar));
        ...
        // #region ICON
            ...
            (function VBN_CustomICON() {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate(key) || !matchedKey) return;
        ...
        // #endregion
            ...
            (function VBN_Clean() {
                ...
                const FLAG = 'vbnReplaced';
                const DOM_ATTR = 'vbn-trace';
                const DOM_ATTR_VALUE = 'true';
                
                const RULES = [ 
                    
                    ['AGEGQEAGE', 'XXNXX'], ['WHWRHRWBR', 'XXNXX'],
                    ['小姐姐', '小美女'], ['小灵', '小玲'], ['灵儿', '玲儿'], ['刘', '隆'],
                    ['FAGEQGHQH', 'XXNXX'], ['WRAWRAWRJJRERAJERAJAEJAEJEARAE', 'XXNXX'],
                    
                    [['白丝', '白色丝袜',], '灰丝'], [['裤袜', '褲襪', '連褲襪'], '丝袜'],
                    
                    [['弟弟', '表弟们', '表弟', '姐夫'], '纵鸣'], [['大嫂', '嫂子', '嫂嫂'], '堂姐'], 
                    [['HAFQ', '屎尿',], 'XXNXX'], [['羞辱', '绿母', '为儿子', '为了儿子', '母狗', ], 'XXXXX'], [['3p',], ' 纵鸣猛肏 '],
                    [['男大学生', '男大', '野爹', '体育生', '学弟',], '纵鸣'], [['贱逼', '母猪', '婊子',], 'XXXXX'],
                    [['ASGHQEHEQH',], 'XXNXX'], [['打桩机',], 'XXNXX'],
                    
                ];
                
                const compiledRules = [];
                RULES.forEach(rule => {
                    const sources = Array.isArray(rule[0]) ? rule[0] : [rule[0]];
                    const replacement = rule[1];
                    sources.forEach(src => {
                        compiledRules.push({
                            regex: new RegExp(src.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'),
                            replacement,
                            target: src
                        });
                    });
                ...
                const shouldSkip = n => {
                    ...
                    return !p || p.nodeName === 'SCRIPT' || p.nodeName === 'STYLE' || p.isContentEditable ||
                        p.nodeName === 'INPUT' || p.nodeName === 'TEXTAREA';
                };
                const containsTarget = text => compiledRules.some(r => text.includes(r.target));
                const apply = node => {
                    let v = node.nodeValue, changed = false;
                    compiledRules.forEach(r => { if (v.includes(r.target)) { v = v.replace(r.regex, r.replacement); changed = true; } });
                    if (changed) { node.nodeValue = v; }
                    return changed;
                };
                function replaceIn(root) {
                    const w = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
                        acceptNode: n => n[FLAG] || shouldSkip(n) || !containsTarget(n.nodeValue) ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT
                    });
                    let n;
                    while (n = w.nextNode()) {
                        n[FLAG] = true;
                        if (apply(n) && n.parentNode.nodeType === 1 && !n.parentNode.hasAttribute(DOM_ATTR)) {
                            n.parentNode.setAttribute(DOM_ATTR, DOM_ATTR_VALUE);
                        }
                    }
                }
                function handleTextNode(n) {
                    if (n[FLAG] || shouldSkip(n) || !containsTarget(n.nodeValue)) return;
                    n[FLAG] = true;
                    if (apply(n) && n.parentNode.nodeType === 1 && !n.parentNode.hasAttribute(DOM_ATTR)) {
                        n.parentNode.setAttribute(DOM_ATTR, DOM_ATTR_VALUE);
                    }
                ...
                function init() {
                    if (document.body) {
                        ...
                        VBN_OBSERVER_CENTER.observeWithKey('vbnTextReplace', document.body, { childList: true, subtree: true }, m => {
                            const nodes = new Set();
                            m.forEach(mu => {
                                if (mu.type === 'childList') mu.addedNodes.forEach(node => {
                        ...
                        new MutationObserver((_, o) => { if (document.body) { o.disconnect(); init(); } })
        ...
        // #region FontStyle
            ...
            VBN_GLOBAL_DISPATCH.activate("FontStyle", {
        ...
        // #region Selection
            ...
            VBN_GLOBAL_DISPATCH.activate("Selection", () => {
        ...
        // #region ScrollBar
            ...
            VBN_GLOBAL_DISPATCH.activate("ScrollBar", () => {
    ...
    // $ ================================================== ↓ Assign
            ...
            VBN_GLOBAL_DISPATCH.activate("Template", {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate("Template")) return;
                ...
                GM_addStyle( /* css */ `
                    ...
                    /* ================================================== ↓ Global */
                        ...
                        html {
                            ...
                            &.vbn body {
                                ...
                                --vbn-XXXX-Width: var(--vbnSpaceWidth02);    /* XXXX */
                                --vbn-XXXX-Hight: var(--vbnSpace2Y);
                                
                                --vbn-XXXX-RadiusS: var(--vbnBaseRadius);
                                --vbn-XXXX-RadiusU: var(--vbnUIRadius);
                                --vbn-XXXX-RadiusM: var(--vbnPanelRadius);
                                --vbn-XXXX-RadiusL: var(--vbnSurfaceRadius);
                    ...
                    /* ================================================== ↓ XXXX */
                        ...
                        html XXXX {
                            ...
                            --vbn-XXXX-XXXX: hsla(var(--vbnPrimaryHSL), .5);
                            
                            .XXXXXXXX {
                                background: hsla(var(--vbnPrimaryHSL), .5);
                            }
        ...
        // #region Baidu
            ...
            VBN_GLOBAL_DISPATCH.activate("Search", {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate("Baidu")) return;
                ...
                GM_addStyle( /* css */ `
                    ...
                    /* ================================================== ↓ Global */
                        ...
                        html {
                            ...
                            &.vbn body {
                                ...
                                --vbn-Baidu-Width: calc(var(--vbnSpaceWidthVW) / 2 );
                                --vbn-Baidu-Hight: var(--vbnSpace2Y);
                                
                                --vbn-Baidu-RadiusS: var(--vbnBaseRadius);
                                --vbn-Baidu-RadiusU: var(--vbnUIRadius);
                                --vbn-Baidu-RadiusM: var(--vbnPanelRadius);
                                --vbn-Baidu-RadiusL: var(--vbnSurfaceRadius);
                                
                                --vbn-Baidu-Repair: -6px;
                                --vbn-Baidu-KeyWord: #3978d0;
                                
                            }
                            
                            .wrapper_new { background: var(--vbnBase09HEX);}
                            
                            /* ========== ↓ 隐藏模块 */
                            
                                #searchTag,                                                                   /* 顶部 - 搜索关联词 */
                                #rs_new .c-color-t.rs-label_ihUhK,                                            /* 底部 - 相关搜索 标题 */
                                #wrapper #container #content_right,                                           /* 内容区 - 右侧板块 */
                                #wrapper #container .c-container[tpl="note_lead"],                            /* 内容区 - 模块 精选笔记 */
                                #wrapper #container .c-container[tpl="yl_vd_generic_new"],                    /* 内容区 - 模块 电影 */
                                #wrapper #container .c-container[tpl="recommend_list"] .c-color-t.title_Zinx- /* 内容区 - 大家还搜索了 标题 */ {
                                    display: none;
                                }
                                
                            /* ========== ↓ 移除下划线 */
                            
                                .doc-title_3Jos6 a:hover,
                                .doc-title_3Jos6 a:hover em {
                                    text-decoration: none !important;
                                }
                                
                                a, em, :link, a:hover, em:hover, a:hover em, a.cos-link:hover,
                                #wrapper #container .c-container .t *,
                                #wrapper #container .c-container .c-title *,
                                #wrapper #container .c-container .cosc-title-a :hover,
                                #wrapper #container .c-container .cosc-title-a :hover em,
                                #wrapper #container .c-container .cosc-title-slot,
                                #wrapper #container .c-container .cosc-title-slot *,
                                #wrapper #container .c-container .cosc-title-slot:hover,
                                #wrapper #container .c-container .cosc-title-slot:hover *,
                                #wrapper #container .c-container .title_7Iz74 .cos-link,
                                #wrapper #container .c-container ._sc-title_1g9za_66 *,
                                #wrapper #container .c-container ._sc-title_1g9za_66 :hover,
                                #wrapper #container .c-container ._sc-title_1g9za_66 :hover *,
                                #wrapper #container .c-container .video-main-title_S_LlQ:hover .title-default_518ig,
                                #wrapper #container .c-container .site-link_1JEbM .custom-underline a .cosc-title-slot:hover,
                                #wrapper #container .c-container ._head-title_1ml43_142 ._link_1ml43_146:hover ._paragraph_1ml43_156 {
                                    text-decoration: none;
                                }
                                
                            /* ========== ↓ 内容区图片尺寸 */
                            
                                .c-container[tpl="ai_ecology"] .cos-row-col-12 > .cos-col-6,                           /* 轻秒图片转换器 */
                                .c-container .cos-swiper-list .cos-swiper-item[style*="width: calc(33% - 5px)"],       /* 百度有驾 */
                                .c-container .video-wrap_7yrJ0 .cos-swiper-list .cos-swiper-item[style*="width: 34%;"] /* 百度翻译 */ {
                                    width: 15% !important;
                                }
                                .c-container .cos-row > .cos-col[style*="width:25%"],                                      /* 百度百科 */
                                #wrapper #container .c-container[tpl="sg_kg_entity_san"] .cos-row.cos-gutter > .cos-col-3, /* 百度百科 */
                                .c-container[tpl="nvl_bookstore_san"] .cos-row-col-12 > .cos-col-3,                        /* 在线阅读 */
                                .c-container .cos-row.bottom-gap_2aWpR.single-image_6zdhC > .cos-col[style*="width:24.5%"] /* 电影 封面 */ {
                                    width: 10% !important;
                                }
                                
                                .c-container[tpl="rel_baikan_index"] .cos-row-col-12 > .cos-col-6:has(.poster_2AaVN) { width: 20%; margin-right: 20px; }
                            ...
                            /* ========== ↓ 原生模块边框 */
                                ...
                                .pc-fresh-smooth .jr-exrate-pc-wrapper_476iT {
                                    outline: transparent;
                                }
                    ...
                    /* ================================================== ↓ Content */
                        ...
                        /* ========== ↓ 组件 */
                            ...
                            html #wrapper #container .c-container {
                                
                                /* ========== ↓ 计算器 */
                                &[tpl="ms_new_calc"] { vbn { top: var(); }
                                ...
                                /* ========== ↓ 日历 */
                                &[tpl="ms_calendar_san"] { vbn { top: var(); }
                                ...
                                /* ========== ↓ 百度时间 */
                                &[tpl="world_time_san"] { vbn { top: var(); }
                                ...
                                /* ========== ↓ 汇率换算 */
                                &[tpl="jr_exrate_san"] { vbn { top: var(); }
                                
                                    .aladdin_3Tvaz .bg-header_G63NE { top: calc(-10px + var(--vbn-Baidu-Repair)); }
                                    
                                }
                                
                                /* ========== ↓ 单位换算 */
                                &[tpl="ms_measures_san"] { vbn { top: var(); }
                        ...
                        /* ========== ↓ 模块 */
                            ...
                            html #wrapper #container { vbn { top: var(); }
                                
                                
                                
                                
                                /* ========== ↓ 官方标 */
                                [class*="title-wrapper_"] [class*="prefix-icon_"] { position: absolute; right: 0; transform: scale(.8); }
                    ...
                    /* ================================================== ↓ Head */
                        ...
                        html .wrapper_new #head { 
                            top: 0;
                            width: 100%;
                            background: var(--vbnGlassBase) !important;
                            backdrop-filter: var(--vbnPanelFilter);
                            transition: var(--vbnOftenTransition);
                        }
                        ...
                        html #wrapper #head:not(.s-manhattan-index div, .s_form_nologin div) { vbn { top: var(); }
                            ...
                            /* ========== ↓ 搜索框 */
                                ...
                                .s_ipt_wr { width: var(--vbn-Baidu-Width); }
                                .s_form_fresh { width: inherit; padding: 0; }
                                .bdsug-new { width: var(--vbn-Baidu-Width); }
                                .soutu-env-new .soutu-layer .soutu-url-wrap, .soutu-env-new .soutu-layer #soutu-url-kw { width: var(--vbn-Baidu-Width); }
                    ...
                    /* ================================================== ↓ Content */
                        ...
                        /* ========== ↓ 模块 通用 */
                            ...
                            html #wrapper #container .new-pmd.c-container[tpl="jy_wenku_wenshu"]:hover,        /* 百度文库 */
                            html #wrapper #container .new-pmd.c-container[tpl="med_wenzhen_san"]:hover,        /* 百度健康 */
                            html #wrapper #container .new-pmd.c-container[tpl="med_aigc_guru_san"]:hover,      /* 百度健康 */
                        ...
                        /* ========== ↓ 模块 标题 */
                            ...
                            /* ========== ↓ 标题字号 */
                                ...
                                html a, html a:hover,
                                html a.cos-link, html a.cos-link:hover { color: var(--vbn-Baidu-KeyWord); }
                        ...
                        /* ========== ↓ 模块 百度AI */
                            ...
                            /* ========== ↓ 问答生成 */
                                ...
                                html #wrapper #container .c-container[tpl="new_baikan_index"] {
                                    ...
                                    /* .cosc-card-content { margin-top: var(--vbn-Baidu-Repair); } */
                                    .cosc-card-light-bg:not([tpl="wenda_generate"] div) { top: var(--vbn-Baidu-Repair); }
                                ...
                                html #wrapper #container .c-container[tpl="med_aigc_guru_san"] { vbn { top: var(); }
                                    ...
                                    .medAigcPcCard_29xxv { margin-top: 10px; }
                        ...
                        /* ========== ↓ 模块 百度百科 */
                            ...
                            html #wrapper #container .c-container[tpl="sg_kg_entity_san"] { vbn { top: var(); }
                                ...
                                .button_sx9Ei .content_7L4g2 { color: var(--vbn-Baidu-KeyWord); }
                                .c-span9 { width: 85%; }
                        ...
                        /* ========== ↓ 模块 相关搜索 */
                            ...
                            html #wrapper #container .c-container[tpl="recommend_list"] { vbn { top: var(); }
                                ...
                                .pc-rg-upgrade_2X3zi .item_3WKCf { margin: 5px !important; color: var(--vbn-Baidu-KeyWord); }
                    ...
                    /* ================================================== ↓ Bottom */
                        ...
                        html #wrapper .new-pmd[tpl="app/page"] { vbn { top: var(); }
                            ...
                            .page_2muyV a { color: var(--vbn-Baidu-KeyWord); }
            ...
            VBN_GLOBAL_DISPATCH.activate("Search", {
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            --vbn-Baike-Width: calc(var(--vbnSpaceWidthVW) - 05vw);
                        }
                        ...
                        @media (width >= 2160px) {
                            ...
                            /* ================================================== ↓ XXX */
                                ...
                                .pageWrapper_P6xcA .contentBox_cyrt9 { width: var(--vbn-Baike-Width); }
        ...
        // #region Google
            ...
            VBN_GLOBAL_DISPATCH.activate("Search", {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate("Google")) return;
                
                GM_addStyle( /* css */ `
                
                    #tsf { transition: transform .526s var(--vbnTransitionSoft); }
                    
                    @media (width >= 1920px) { body { --vbn-Google-Width: calc(var(--vbnSpaceWidthVW) - 05vw); } }
                    @media (width <= 1920px) { body { --vbn-Google-Width: calc(var(--vbnSpaceWidthVW) + 05vw); } }
                    ...
                    html:has(body [role="listitem"]:nth-child(1) a[aria-disabled="true"]) {
                        ...
                        &.vbn body {
                            ...
                            --vbn-Google-Search: calc(var(--vbn-Google-Width) / 1.5);
                            --vbn-Google-ICON: 50px;
                            --vbn-Google-Line: #00000009;
                            
                            --vbnRadiusS: var(--vbnBaseRadius);
                            --vbnRadiusU: var(--vbnUIRadius);
                        ...
                        /* ================================================== ↓ Head */
                            ...
                            .tsf {
                                grid-column: 2;
                                justify-self: center;
                                width: var(--vbn-Google-Search);
                                max-width: var(--vbn-Google-Search);
                            ...
                            .wZQcA .BO2cCe, .zLSRge { border-color: var(--vbn-Google-Line); }
                            ...
                            /* ========== ↓ 粘性定位 */
                                ...
                                .minidiv > .NDnoQ.P3mIxe { vbn { top: var(); }
                                    ...
                                    .RNNXgb {
                                        margin: 5px 0 0;
                                        min-height: var(--vbn-Google-ICON);
                                    }
                                    .iblpc, .vOY7J, .XDyW0e, .nDcEnd, .Tg7LZd { min-height: var(--vbn-Google-ICON); }
                            ...
                            /* ========== ↓ 分类 */
                                ...
                                .rQTE8b { max-width: var(--vbn-Google-Search); margin-left: 5.26vw; }
                        ...
                        /* ================================================== ↓ Content */
                            ...
                            #center_col { width: var(--vbn-Google-Width); }
                            ...
                            /* ========== ↓ 模块 通用 */
                                ...
                                /* ========== ↓ 通用模块 含图片 */
                                    ...
                                    .srKDX > .kb0PBd.ieodic.jGGQ5e { width: calc(var(--vbn-Google-Width) - 20px); }
                                    .srKDX > .kb0PBd.LnCrMe {
                                        position: absolute;
                                        right: 20px;
                                        bottom: -10%;
                            ...
                            /* ========== ↓ 模块 官方词条 */
                                ...
                                .GCSyeb { margin: 0 auto; width: calc(var(--vbn-Google-Width) - 260px); background: var(--vbn-Google-Line); }
                            ...
                            /* ========== ↓ 模块 图片 */
                                ...
                                .Lv2Cle .eA0Zlc.mkpRId, .cakeVe .m3LIae {
                                    width: calc(var(--vbn-Google-Width) / 5);
                                    max-width: 126px;
                                }
                                .Bi9oQd { background: var(--vbn-Google-Line); }
                            ...
                            /* ========== ↓ 模块 影片 */
                                ...
                                .sHEJob { border-top: none; /* max-width: calc(var(--vbn-Google-Width) / 3 - 52px); */ }
                            ...
                            /* ========== ↓ 模块 相关问题 */
                                ...
                                .akqY6 { background: var(--vbn-Google-Line); }
                                .iRPzcb { border-color: var(--vbn-Google-Line); }
                        ...
                        /* ================================================== ↓ Bottom */
                            ...
                            /* ========== ↓ 底部 */
                                ...
                                #fbarcnt { width: var(--vbn-Google-Width); margin: 0 auto; }
                                #fbar {
                                    opacity: .6;
                ...
                /* ========== QuickRead */
                    ...
                    (function VBN_Google_QuickRead() {
                        ...
                        if (!VBN_GLOBAL_DISPATCH.activate("QuickRead")) return;
            ...
            VBN_GLOBAL_DISPATCH.activate("Google", {
        ...
        // #region Bing
            ...
            VBN_GLOBAL_DISPATCH.activate("Search", {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate("Bing")) return;
        ...
        // #region ComfyUI
            ...
            VBN_GLOBAL_DISPATCH.activate("ComfyUI", {
                ...
                (function VBN_ComfyUI_manager() {
                    ...
                    if (!VBN_GLOBAL_DISPATCH.activate("manager")) return;
                ...
                (function VBN_ComfyUI_crystools() {
                    ...
                    if (!VBN_GLOBAL_DISPATCH.activate("crystools")) return;
                ...
                (function VBN_ComfyUI_rgthree() {
                    ...
                    if (!VBN_GLOBAL_DISPATCH.activate("rgthree")) return;
        ...
        // #region DeepSeek
            ...
            VBN_GLOBAL_DISPATCH.activate("Chat", {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate("DeepSeek")) return;
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            ...
                            --vbn-DeepSeek-Width: var(--vbnSpaceWidth02);
                            --vbn-DeepSeek-Hight: var(--vbnSpace2Y);
                        ...
                        /* ========== ↓ 首页搜索框 */
                        ._9a2f8e4 { max-width: calc(var(--vbn-DeepSeek-Width) + 5vw); }
                        
                        /* ========== ↓ 输入框 */
                        ._77cefa5 ._27c9245, ._77cefa5 .b13855df { min-height: 92px; }
                        
                        /* ========== ↓ 聊天容器 */
                        .ds-virtual-list { --message-list-max-width: calc(var(--vbn-DeepSeek-Width) + 5vw); }
                        
                        @media (width <= 1920px) {
                            ._9a2f8e4 { max-width: calc(var(--vbn-DeepSeek-Width) + 20vw); }
                            .ds-virtual-list { --message-list-max-width: calc(var(--vbn-DeepSeek-Width) + 20vw); }
                        }
                        @media (width <= 1260px) {
                            ._9a2f8e4 { max-width: calc(var(--vbn-DeepSeek-Width) + 40vw); }
                            .ds-virtual-list { --message-list-max-width: calc(var(--vbn-DeepSeek-Width) + 40vw); }
                        }
                        ...
                        /* ========== ↓ 代码块 */
                        &.vbn .md-code-block > pre, .md-code-block > pre * {
                            font-family: var(--vbnCodeFont) !important;
                            font-size: 12px;
                        }
        ...
        // #region ChatGPT
            ...
            VBN_GLOBAL_DISPATCH.activate("Chat", {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate("ChatGPT")) return;
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            ...
                            --vbn-ChatGPT-Width: var(--vbnSpaceWidth02);
        ...
        // #region DOUBAO
            ...
            VBN_GLOBAL_DISPATCH.activate("Chat", {
                ...
                if (!VBN_GLOBAL_DISPATCH.activate("DOUBAO")) return;
        ...
        // #region YUQUE
            ...
            VBN_GLOBAL_DISPATCH.activate("YUQUE", {
                ...
                GM_addStyle( /* css */ `
                    ...
                    /* ================================================== ↓ Global */
                        ...
                        html {
                            ...
                            &.vbn body {
                                ...
                                --vbn-YUQUE-Width: 260px;                                                    /* 实际内容宽度为 原 750px + 设置的宽度 */
                                --vbn-YUQUE-Offset: -60px;                                                   /* 因原有左右侧边栏宽度不一 可设置向右偏移 调整至视觉居中 */
                                --vbn-YUQUE-Margin: var(--vbn-YUQUE-Width) / 2 + var(--vbn-YUQUE-Offset);    /* 官方值 XXX + 宽度的一半 + 偏移值 */
                                ...
                                @media (width <= 2460px) { --vbn-YUQUE-Width: 260px; --vbn-YUQUE-Offset: 0px; }
                                @media (width <= 1920px) { --vbn-YUQUE-Width: 100px; --vbn-YUQUE-Offset: 0px; }
                                @media (width <= 1202px) { --vbn-YUQUE-Width: 000px; --vbn-YUQUE-Offset: 0px; }
                            ...
                            /* ========== ↓ 右侧边栏 文档导航 */
                                ...
                                .ne-toc-sidebar .ne-toc-view {
                                    ...
                                    border-radius: var(--vbnRadiusM) var(--vbnRadiusU) var(--vbnRadiusU) var(--vbnRadiusM);
                                }
                            ...
                            /* ========== ↓ 疑似 表格/数据表全宽 「侧边栏 开启时」 */
                                ...
                                [class^="DocReader-module_wrapper_"][data-doc-layout="fixed"]:is( [data-doc-sidebar="true"], [data-doc-toc="true"] )
                                    ...
                                    width: calc(100% - 305px + var(--vbn-YUQUE-Width));
                                }
                    ...
                    /* ========== ↓ 内容宽度 */
                        ...
                        html .ne-viewer-body > *, html [class*="DocReader-module_header"] { transition: all .92s var(--vbnTransitionSoft) }
                        ...
                        /* ========== ↓ 阅读模式 */
                            ...
                            html [class*="DocReader-module_info"] {
                                max-width: calc(850px + var(--vbn-YUQUE-Margin));
                            }
                            ...
                            html .ne-doc-major-viewer .ne-viewer-layout-mode-fixed .ne-viewer-body > * {
                                max-width: calc(750px + var(--vbn-YUQUE-Width));
                            }
                            ...
                            html .article-content .ne-doc-major-viewer .ne-viewer-layout-mode-fixed .ne-viewer-body > * {
                                margin-right: calc(var(--viewer-center-align-right) - var(--vbn-YUQUE-Margin)) !important;
                            }
                            /* ========== ↓ 代码块 */
                            ...
                            html .article-content .ne-doc-major-viewer .ne-viewer-layout-mode-fixed .ne-viewer-body > ne-hole {
                                width: calc(100% - var(--viewer-center-align-value) + var(--vbn-YUQUE-Width));
                            }
                        ...
                        /* ========== ↓ 编辑模式 */
                            ...
                            html .ne-doc-major-editor .ne-editor.layout-read-write:not(.ne-layout-mode-adapt) .ne-editor-extra-box {
                                max-width: calc(890px + var(--vbn-YUQUE-Margin));
                            }
                            ...
                            html .ne-doc-major-editor .ne-editor.layout-read-write:not(.ne-layout-mode-adapt) .ne-engine>:not(.ne-full-width) {
                                margin-right: calc(var(--center-editor-margin-right) - var(--vbn-YUQUE-Margin)) !important;
                                max-width: calc(750px + var(--vbn-YUQUE-Width));
                            }
                            ...
                            html .ne-doc-major-editor .ne-layout-mode-fixed.ne-normal-toc:not(.ne-ui-sidebar-visible).ne-viewport-size-toc-XXL .ne-engine ne-table-hole {
                                max-width: calc(752px + var(--vbn-YUQUE-Width));
                            }
                ...
                (function VBN_YUQUE_vbnTable() {
                    ...
                    if (!VBN_GLOBAL_DISPATCH.activate("vbnTable")) return;
                ...
                (function VBN_YUQUE_vbnGrid() {
                    ...
                    if (!VBN_GLOBAL_DISPATCH.activate("vbnGrid")) return;
        ...
        // #region TickTick
            ...
            VBN_GLOBAL_DISPATCH.activate("TickTick", {
        ...
        // #region Youtube
            ...
            VBN_GLOBAL_DISPATCH.activate("Youtube", {
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            ...
                            --vbn-Youtube-Video: 6;                       /* 显示数量 Video */
                            --vbn-Youtube-Shots: 8;                       /* 显示数量 Shots */
                            --vbn-Youtube-Blur: var(--vbnPanelFilter);    /* 播放器 控件背景模糊值*/
                        ...
                        @media (width >= 1500px) {
                            ...
                            ytd-rich-grid-renderer[is-default-grid] ytd-rich-item-renderer[rendered-from-rich-grid] {
                                --ytd-rich-grid-items-per-row: var(--vbn-Youtube-Video) !important;
                            }
                            
                            /* ========== ↓ Shots */
                            ytd-rich-shelf-renderer[is-shorts] ytd-rich-item-renderer[items-per-row][is-slim-media] {
                                --ytd-rich-grid-items-per-row: var(--vbn-Youtube-Shots) !important;
                            }
                        ...
                        /* ========== ↓ 播放器 */
                            ...
                            .ytp-delhi-modern .ytp-popup,                                                             /* 设定背景 */
                            .ytp-delhi-modern .ytp-chrome-controls .ytp-right-controls,                               /* 设定条 */
                            .ytp-delhi-modern .ytp-time-wrapper:not(.ytp-miniplayer-ui *),                            /* 播放时间 */
                            .ytp-delhi-modern.ytp-delhi-horizontal-volume-controls .ytp-volume-area,                  /* 音量 */
                            .ytp-delhi-modern.ytp-delhi-modern-compact-controls .ytp-chrome-controls .ytp-play-button /* 播放 */ {
                                backdrop-filter: var(--vbn-Youtube-Blur);
                            }
        ...
        // #region Bilibili
            ...
            VBN_GLOBAL_DISPATCH.activate("Bilibili", {
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        &.vbn body {
                            ...
                            --vbn-Bili-Blur: var(--vbnPanelFilter);
                            --vbn-Bili-PlayList: calc(40vh + 2vh + 1vh);    /* 播放列表高度 */
                            --vbn-Bili-Base: hsla(var(--vbnAccentHSL-H), 10%, 20%, 1);          /* 播放器 底色 */
                            --vbn-Bili-Shadow: var(--vbnGlassShadow);
                        ...
                        &.vbn .search-input-container .search-fixed-header {
                            box-shadow: var(--vbn-Bili-Shadow);
                        }
                        ...
                        /* ========== ↓ 播放器 */
                            ...
                            #playerWrap { border-radius: var(--vbnRadiusU); }
                            ...
                            .bpx-player-video-wrap video {
                                background: 
                                    ...
                                    linear-gradient( 
                                        ...
                                        var(--vbn-Bili-Base) 0%,
                                        hsla(0, 0%, 0%, 1) 50%,
                                        var(--vbn-Bili-Base) 100%
                                    )
                                ;
                            }
                            
                            /* ========== ↓ 弹幕输入 */
                            .bpx-player-sending-bar .bpx-player-video-inputbar { backdrop-filter: var(--vbn-Bili-Blur); }
                        ...
                        /* ========== ↓ 播放列表 */
                            ...
                            .simple-base-item.normal.active { border-radius: var(--vbnRadiusS); }
                            ...
                            &.vbn .video-container-v1 .video-pod .video-pod__body {
                                max-height: var(--vbn-Bili-PlayList);
                            }
    ...
    // $ ================================================== ↓ Micro
        ...
        // #region Micro
            ...
            VBN_GLOBAL_DISPATCH.activate("Eagle", () => {
            ...
            VBN_GLOBAL_DISPATCH.activate("NetDisk_Check", () => {
                ...
                GM_addStyle( /* css */ `
                    ...
                    html {
                        ...
                        /* ========== 带提取码 */
                        .one-pan-tip-lock::before { background-image: var(--vbnICON-URL-Safety); }
                        
                        /* ========== 夸克 */
                        .one-pan-tip-partial::before { background-image: var(--vbnICON-URL-Info); }
        ...
        // #region Other
            ...
            /* ========== Notion */
                ...
                VBN_GLOBAL_DISPATCH.activate("Other", {
                    ...
                    GM_addStyle( /* css */ `
                        ...
                        html {
                            ...
                            &.vbn body {
                                --vbn-Notion-Width: var(--vbnSpaceWidth02);
                            }
                            
                            .layout.layout-reskin-wider {
                                --content-width: minmax(auto, var(--vbn-Notion-Width));
                            }
                    ...
                    GM_addStyle( /* css */ `
                        ...
                        html {
                            ...
                            &.vbn body {
                                ...
                                --vbn-Abbr-Width: 1260px;
                                --vbn-Abbr-Aside: 330px;
                                --vbn-Abbr-Shadow: 0 .5em 2em 0 hsla(184, 22%, 14%, .12);
                            ...
                            #main, .content-top, #header-int, .page-top-search { max-width: var(--vbn-Abbr-Width); }
                            
                            .page-word-search { width: calc(100% - var(--vbn-Abbr-Aside)); }
                            #content-aside { max-width: var(--vbn-Abbr-Aside); }
                            ...
                            #content-body > section.split > div {
                                box-shadow: var(--vbn-Abbr-Shadow);
                            }
                            
                            @media (width >= 768px) {
                                
                                #main , #header, #footer { min-width: var(--vbn-Abbr-Width); }
                                
                                .col-sm-8 { width: calc(100% - var(--vbn-Abbr-Aside) ); }
                                .col-sm-push-4 { left: calc(var(--vbn-Abbr-Aside)); }
                                .col-sm-pull-8 { right: calc(100% - var(--vbn-Abbr-Aside)); }
            ...
            /* ========== RadBook */
                ...
                VBN_GLOBAL_DISPATCH.activate("Other", {
                    ...
                    GM_addStyle( /* css */ `
                        ...
                        /* ================================================== ↓ Global */
                            ...
                            html {
                                ...
                                .channel-list .channel-list-content {
                                    overflow: hidden;
                    ...
                    (function VBN_RadBook_ReLimits() {
                        
                        VBN_REMOVE_LIMITS.apply({
                            key: "ctrl",
                            ReLimits: "all",
                        });
