...
// @namespace                 Global.Custom-VSO
// @homepageURL               https://greasyfork.org/scripts/579569
// @compatible                Chrome
// @license                   Apache-2.0
    ...
    // $ ================================================== ↓ Global
        ...
        // #region Basics
            ...
            const VBN_MATCH_RULE = (() => {
                ...
                function toRegex(pattern) {
                    if (cacheRegex.has(pattern)) return cacheRegex.get(pattern);
                    const escaped = pattern
                        .replace(/([.+^${}()|[\]\\])/g, '\\$1')
                        .replace(/\*/g, '.*')
                        .replace(/^https?:/, 'https?:');
                    const regex = new RegExp('^' + escaped + '$', 'i');
                    cacheRegex.set(pattern, regex);
                    return regex;
                }
            ...
            const VBN_MODULE_STORE = (() => {
                ...
                for (const [groupName, groupData] of Object.entries(VBN_GLOBAL_MODULES)) {
                    ...
                    for (const [modKey, modRaw] of Object.entries(groupData)) {
                        ...
                        for (const [subKey, subRaw] of Object.entries(modRaw)) {
                            if (
                                ["match", "state", "default", "feature", "debug", "tips", "name", "fold"].includes(subKey)
                ...
                function getGroup(key) {
                    if (cacheGroup.has(key)) return cacheGroup.get(key);
                    for (const [groupName, modules] of Object.entries(tree)) {
                        if (key in modules) {
                            cacheGroup.set(key, groupName);
                            return groupName;
                        }
                    ...
                    cacheGroup.set(key, null);
                    return null;
                }
                
                function getChain(key, { includeGroup = false } = {}) {
                    if (cacheChain.has(key)) return cacheChain.get(key);
                    const chain = [];
            ...
            const VBN_ACTIVATION = (() => {
                ...
                function findMatch(mod) {
                    ...
                    if (cacheMatch.has(mod.key)) return cacheMatch.get(mod.key);
                    ...
                    cacheMatch.set(mod.key, result);
                    return result;
                }
                
                function getValue(key, fallback = true) {
                    if (cacheValue.has(key)) return cacheValue.get(key);
                    const val = GM_getValue(key, fallback);
                    cacheValue.set(key, val);
                    return val;
                }
            ...
            (function VBN_Options() {
                ...
                function buildPanel() {
                    ...
                    resetBtn.addEventListener("click", () => {
                        ...
                        if (typeof VBN_ACTIVATION?.clearCache === "function") {
                            VBN_ACTIVATION.clearCache();
                        }
        ...
        // #region ICON
            ...
            (function VBN_CustomICON() {
                ...
                // 配置映射关系 "网址" => "变量" 「 可转至315行左右配置 支持格式 JPG PNG SVG base64... 」
                const vbnICON_Map = {
                    ...
                    /* 
                        ...
                        html .comfyui-menu #crystools-monitors-root { vbn { top: var(); }
                            ...
                            .crystools-monitor[class*="Crystools.ShowCpu"] { 
                                order: 2 !important; .crystools-slider { background: #b5e70d !important; } /* #00F29C */
                            }
                            ...
                            .crystools-monitor[class*="Crystools.ShowGpuVramZero"] { 
                                order: 4 !important; .crystools-slider { background: #8576f7 !important; }
                            }
                            .crystools-monitor[class*="Crystools.ShowGpuTemperatureZero"] { 
                                order: 5 !important; .crystools-slider { background: #ff3cac !important; }
                            }
