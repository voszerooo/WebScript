...
// @include                   *://localhost:8188/*
// @include                   *://127.0.0.1:8188/*
...
(function pkgVBN_GlobalCustom() {
    ...
    // $ ================================================== ↓ Global
        ...
        // #region Config
            ...
            /**
             * @type {Object}
             * @property {boolean} debug       - 全局调试 | 开启后控制台将输出当前站点 各功能的运行状态
             * @property {Object} siteBlock    - 站点阻塞 | 功能若与原始站点产生冲突 可在相应功能组添加屏蔽站点 进行紧急阻塞
             */
        ...
        // #region Modules
             ...
             * 
             * @type {Object}
             * @property {Object} Global                                 - 模块组 | 命名将对应 Options 中的 分组名
             * @property {string} Global.group                           - 分组唯一标识
             * 
             * @property {string} [name=" "]                             - 名称 | 命名将对应 Options 中的 功能名
             * @property {string} [feature=" "]                          - 功能描述 | 将用于提示
             * @property {boolean} [state=true]                          - 当前状态
             * @property {boolean} [default=true]                        - 默认状态
             * @property {boolean} [tips=false]                          - 开关时 是否提示功能描述
             * @property {boolean} [debug=false]                         - 调试模式
             * @property {boolean} [error=false]                         - 是否处于报错状态（运行中可动态标记）
             * 
             * @property {Object} [match]                                - 定义 URL 规则列表 「支持 * 通配符」
             * @property {string[]} [match.include=[]]                   - 匹配
             * @property {string[]} [match.exclude=[]]                   - 排除
             * 
             */
            ...
            const VBN_MATCH_LOGIC = (() => {
                ...
                function activate(key, options = {}) {
                    ...
                    if (VBN_GLOBAL_CONFIG.debug) {
                        console.log(`[VBN DEBUG] 激活检测模块: ${key} 当前URL: ${url}`);
                    }
                    
                    for (const [groupName, blockPatterns] of Object.entries(VBN_GLOBAL_CONFIG.siteBlock)) {
                        if (blockPatterns.length > 0 && module.group === groupName) {  // 👈 只阻止当前模块所在组
                            const isBlocked = VBN_MATCH_RULE.match({ include: blockPatterns, url });
                            if (isBlocked) {
                                if (VBN_GLOBAL_CONFIG.debug) {
                                    console.warn(`[VBN BLOCK] 模块组 "${groupName}" 在 URL "${url}" 下被屏蔽 模块 "${key}" 停止激活`);
                                }
                                return false;
                            }
                        }
                    ...
                    if (isActive && module.debug === true) {
                        ...
                        VBN_GLOBAL_LOG.log(key, `ON  |  Time ${timeSinceStart} ms`);
                    }
                            ...
                            steps.forEach((step, i) => {
                                ...
                                setTimeout(() => {
                                    ...
                                    try {
                                        ...
                                        if (el) {
                                            runAction(el, action);
                                            VBN_GLOBAL_LOG.log(moduleMark, `Step${i + 1} ✅ ${action} - ${target}`, { debug });
                                        } else {
                                            VBN_GLOBAL_LOG.warn(moduleMark, `Step${i + 1} ❓ None - ${target}`, { debug });
                                            if (strict) {
                                                shouldBreak = true;
                                                VBN_GLOBAL_LOG.error(moduleMark, `Step${i + 1} ❌ Break`, { debug });
                                            }
                                        }
                                    } catch (e) {
                                        VBN_GLOBAL_LOG.error(moduleMark, `Step${i + 1} ❌ Error - ${e.message}`, { debug });
                                    }
                                }, delayEach);
                            });
                        }, delay);
        ...
        // #region Derive
            ...
            const VBN = {
                ...
                SITE: VBN_SITE_GROUP,
                MODULES: VBN_GLOBAL_MODULES,
                
                RULE: VBN_MATCH_RULE,
                LOGIC: VBN_MATCH_LOGIC,
                LOG: VBN_GLOBAL_LOG,
        ...
        // #region ComfyUI
            ...
            if (VBN_MATCH_LOGIC.activate("AIGC", {
                ...
                GM_addStyle( /* css */ `
                    ...
                    html .comfyui-menu[data-v-1545ebd4] { height: 50px; }
                    ...
                    /* ========== ↓ 右键菜单 */
                        ...
                        /* ========== ↓ 菜单选项 */
                        html .litegraph.litecontextmenu .litemenu-entry {
                            display: flex;
                            align-items: center;
                            padding: 2px;
                            margin: 4px 0px;
                        ...
                        html .litegraph.litecontextmenu .litemenu-entry.submenu,
                        ...
                        html .litemenu-entry.has_submenu::after {
                            ...
                            box-shadow: 2px 0 6px 0px hsla(var(--themeColor), .92);
                            border-radius: 1em;
                        }
                ...
                /* ========== AutoExecute */
                    ...
                    // (function pkgVBN_ComfyUI_AutoExecute() {
                        
                    //     VBN_AUTO_EXECUTE.apply
            ...
            if (VBN_MATCH_LOGIC.activate("YUQUE", {
                ...
                GM_addStyle( /* css */ `
                    ...
                    /* ========== ↓ 右侧边栏 文档导航 */
                        ...
                        html .sidePanel-module_panel_Vr-DC { background-color: transparent; }
                    ...
                    /* ========== ↓ 内容宽度 */
                        ...
                        @media (min-width: 1500px) {
                            ...
                            #main {
                                ...
                                --vbnWidthOffset: -60px;  /* 因 原有左右侧边栏宽度不一 可偏移调整至视觉居中 */
                            }
                        ...
                        /* ========== ↓ 编辑模式 */
                            ...
                            html .ne-doc-major-editor .ne-editor.layout-read-write:not(.ne-layout-mode-adapt) .ne-editor-extra-box {
                                max-width: calc(890px + var(--vbnWidth) / 2);
                            }
                            
                            /* ========== ↓ 内容 */
                            html .ne-doc-major-editor .ne-editor.layout-read-write:not(.ne-layout-mode-adapt) .ne-engine>:not(.ne-full-width) {
                                margin-right: calc(var(--center-editor-margin-right) - var(--vbnWidth) / 2 ) !important;
                                max-width: calc(750px + var(--vbnWidth));
                            }
                ...
                /* ========== widthAdjust */
                    ...
                    // (function pkgVBN_YUQUE_widthAdjust() {
                        
                    //     const selectorAside = ".ReaderLayout-module_aside_WX6k7";
                    //     const offsetTarget = [
                    //         "#doc-reader-content",
                    //     ];
                    //     const offsetExtra = 0;
                    ...
                    //         if (!aside) return;
                    //         const width = parseFloat(getComputedStyle(aside).width || "0");
                    //         for (const selector of offsetTarget) {
                    //             const target = document.querySelector(selector);
                    //             if (target) {
                    //                 if (!target.style.transition) {
                    //                     target.style.transition = "all .526s var(--vbnTransitionSoft)";
                    //                 }
                    //                 target.style.marginLeft = width > 0 ? `${width + offsetExtra}px` : "0px";
                    //             }
                    //         }
                    //     }
                    ...
                    //         const aside = document.querySelector(selectorAside);
                    //         if (!aside) {
                    //             setTimeout(resizeObserver, 1260);
                    //             return;
                    //         }
                    //         offsetAdjust();
                    //         new ResizeObserver(offsetAdjust).observe(aside);
                    //     }
